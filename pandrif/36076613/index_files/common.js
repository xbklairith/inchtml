...
$(document).ready(function() {
	...
	$(document).on('click', 'a[rel="nofollow"]', function(){
		...
		var redi_href= url+base64EncodeUnicode(link); // encodeURIComponent  คือการ endoce(url) 
	...
	$(document).on('mouseover, mouseout', 'a[rel="nofollow"]', function() {
		...
		link = link.replace(url, '');
		link = url+base64EncodeUnicode(link);  //base64EncodeUnicode(link);
		$(this).attr('href', link);
	});
 ...
 **/
    ...
    $("#search-box").autocomplete({
        ...
        select: function (event, ui) {
            ...
            if(ui.item !== undefined)
            {
                url = "https://pantip.com/tag/"+ui.item.label;    
            }
            else
            {
                url = "https://pantip.com/search.php?s=a&nms=1&q="+term+"&sa=Smart+Search";
            }
    ...
    $("#search-box-float").autocomplete({
        ...
        $("#search-box-float").keypress(function(event){
